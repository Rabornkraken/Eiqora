...
def fetch_entry_price(conn, symbol: str, entry_time: datetime) -> float | None:
    with conn.cursor() as cur:
        cur.execute(
            """
            SELECT close
...
def fetch_future_bars(conn, symbol: str, after_time: datetime) -> list[tuple]:
    with conn.cursor() as cur:
        cur.execute(
            """
    ...
    with get_connection() as conn:
        for symbol, check_time in test_bars:
            ...
            for trigger in triggers:
                ...
                if not entry_price:
                    entry_price = fetch_entry_price(conn, symbol, check_time)
                if not entry_price:
